...
downloading: false,	// Indica si ya existe una solicitud pendiente
...
showImage: function(index, add) {
	let currentIndex, lastIndex, ani, urlImg,
	...
	if (imgBot.hasAttribute("src")) {
		this.autoHandler = setTimeout(function() {
					Carousel.showImage(1, true)
				}, 3000);
		return true;
	}

	// No generar nuevas peticiones si hay pendientes
	if (this.downloading) {
		return true;
	}
	...
	$.ajax({
		...
		.fail(function() {
			if (currentIndex === Carousel.slideIndex) {
				imgBot.alt = "No se pudo cargar la imagen";
				setTimeout(function() {
					imgBot.style.display = "block";
				}, 500);
			}
